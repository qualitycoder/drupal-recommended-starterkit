...
class ScriptHandler {
  ...
  public static function createRequiredFiles(Event $event) {
    ...
    if (!$fs->exists($drupalRoot . '/sites/default/settings.php') && $fs->exists($drupalRoot . '/sites/default/default.settings.php')) {
      ...
      $settings['settings']['config_sync_directory'] = (object) [
        'value' => Path::makeRelative($drupalRoot . '/../config/sync', $drupalRoot),
        'required' => TRUE,
      ];
      ...
      $event->getIO()->write("<info>Settings files is already present.</info>");
    }
  ...
  public static function createDotEnvFile(Event $event) {
    ...
    // Create the .env file.
    if (!$fs->exists($drupalRoot . "/../.env")) {
      $fs->copy($drupalRoot . "/../.env.example", $drupalRoot . "/../.env");
      $fs->chmod($drupalRoot . "/../.env", 0666);
      $event->getIO()->write("Created a dotenv Environment file");
    }
    else {
      $event->getIO()->write("<info>The dotenv Environment file is present.</info>");
    }
  }
  ...
  public static function checkComposerVersion(Event $event) {
    ...
    $io = $event->getIO();
    $io->write("<info>Drupal requires Composer version >= 2.3.6. Checking if the Composer version is compatible.</info>");
    ...
    $io->write("<info>Composer version is compatible.</info>");
  }
